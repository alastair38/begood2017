...
// wrap in an if($acf->meeting_date) {} conditional to filter resources that are not meetings if we don't use WP Rest filter plugin
    if( null == get_page_by_title( $title->rendered, OBJECT, 'post' ) ) {



    $post_id = wp_insert_post( array(
  'post_title'	=> $title->rendered,
  'post_type'		=> 'post', // this will be set to projects on the Neurosec site
  'post_status'	=> 'publish', // might want to set this initially to draft
  //'post_parent' => 86 this will be the id of previous meetings project on Neurosec
...
// hook into the post_status_transitions action to run the fetch and import of the Begood json feed. This action should be run whenever Zapier creates a pending project (which is does when a new resource is added on BeGOOD) - this will prevent it from running unnecessarily if someone is drafting an unrelated project page
add_action('pending_projects', 'begood_trigger');
